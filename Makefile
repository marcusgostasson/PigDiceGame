...
pylint:
	@$(call MESSAGE,$@)
	-cd PigDiceGame && $(PYTHON) -m pylint *.py
...
black:
	@$(call MESSAGE,$@)
	 $(PYTHON) -m black PigDiceGame/ test/
...
pydoc:
	@$(call MESSAGE,$@)
	install -d doc/pydoc
	$(PYTHON) -m pydoc -w PigDiceGame/*.py
	mv *.html doc/pydoc

pdoc:
	@$(call MESSAGE,$@)
	pdoc --force --html --output-dir doc/pdoc PigDiceGame/*.py

pyreverse:
	@$(call MESSAGE,$@)
	install -d doc/pyreverse
	pyreverse PigDiceGame/*.py
